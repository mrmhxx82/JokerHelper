...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'بن گلوبال' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] globally banned')
		banall_user(member_id)
      elseif get_cmd == 'انبن از همه گروه ها' then
...
if matches[1]:lower() == 'اخراج' then
  ...
  if matches[1]:lower() == 'بن گلوبال' and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'انبن از همه گروه ها' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
  ...
  if matches[1]:lower() == "گلوبال بن لیست" then -- Global ban list
    return banall_list()
  end
end
